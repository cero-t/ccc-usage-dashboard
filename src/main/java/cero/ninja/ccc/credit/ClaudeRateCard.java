 ...
 *
 * <p>Fast mode ({@code speed = "fast"} on the Claude Code {@code api_request}
 * event) bills every token category at {@link #FAST_MODE_MULTIPLIER}x the
 * standard rate on the models that support it; prompt caching multipliers
 * stack on top, so the whole rate row scales.
 */
@ApplicationScoped
public class ClaudeRateCard {
    ...
    private static final Rates SONNET_5 = new Rates(2.0, 2.50, 0.20, 10.0);
    ...
    static final double FAST_MODE_MULTIPLIER = 2.0;

    public Costs compute(String model, Long inputTokens, Long cacheCreationTokens,
                         Long cacheReadTokens, Long outputTokens) {
        return compute(model, null, inputTokens, cacheCreationTokens, cacheReadTokens, outputTokens);
    }

    public Costs compute(String model, String speed, Long inputTokens, Long cacheCreationTokens,
                         Long cacheReadTokens, Long outputTokens) {
        Rates rates = rateFor(model, speed);
    ...
    public Rates rateFor(String model, String speed) {
        Rates rates = rateFor(model);
        return isFast(speed) && supportsFastMode(model) ? rates.times(FAST_MODE_MULTIPLIER) : rates;
    }
    ...
    public Rates rateFor(String model) {
        ...
        if (m.contains("fable-5-1") || m.contains("fable-5.1")
                || m.contains("mythos-5-1") || m.contains("mythos-5.1")) {
            return FABLE_5_1;
        }
    ...
    private static boolean isFast(String speed) {
        return speed != null && speed.strip().equalsIgnoreCase("fast");
    }

    // Fast mode runs on Opus 5.5, Opus 5 and Opus 4.8 only. Opus 4.6 requests
    // asking for fast mode run and bill at standard speed.
    private static boolean supportsFastMode(String model) {
        if (model == null) {
            return false;
        }
        String m = model.strip().toLowerCase();
        return m.contains("opus-5") || m.contains("opus-4-8") || m.contains("opus-4.8");
    }
    ...
    public record Rates(double inputPerMt, double cacheWrite5mPerMt,
        ...
        Rates times(double multiplier) {
            return new Rates(inputPerMt * multiplier, cacheWrite5mPerMt * multiplier,
                    cacheReadPerMt * multiplier, outputPerMt * multiplier);
        }
    }
